...
def display_stocks_overview():
    ...
    with st.container():
        ...
        with net_income_chart:          
            ...
            # Filter the DataFrame to include only years from 2020 bis 2023
            ...
            # Add bars for Net Income (added first)
            ...
            # Add bars for Total Revenue (added second to be stacked on Net Income)
            fig_income_chart.add_trace(go.Bar(
                name='Total Revenue',
                x=filtered_data['Year'],
            ...
            # Add bars for Net Profit Margin
            fig_income_chart.add_trace(go.Bar(
                name='Net Profit Margin',
                x=filtered_data['Year'],
                y=[0] * len(filtered_data['Year']),
            ...
            for idx, (year, net_income) in enumerate(zip(filtered_data['Year'], filtered_data['Net Income Common Stockholders'])):
                annotations.append(dict(
                    x=year,
                    y=net_income + (filtered_data['Net Income Common Stockholders'].max() * 0.05),  # Adjust y position as needed
                    text=f"{net_profit_margin_ratio.iloc[idx]}%",
                    showarrow=False,
                    font=dict(size=17, color='orange'),
                    align='center',
                    bordercolor='white',
                    borderwidth=1,
                    borderpad=3,
            ...
            fig_income_chart.update_layout(
                ...
                showlegend=True,
                legend=dict(traceorder='normal',  # Ensure legend order
                            font=dict(color=st.session_state.text_color),
                            x=0.5, xanchor='center', y=-0.1, orientation='h'),  # Legend at the bottom
        ...
        with cash_flow_chart:
            ...
            if cash_flow_data.empty:
                ...
                fig_cash_flow.add_trace(go.Bar(
                    ...
                    marker=dict(color='cornflowerblue')  # Customize bar color
                ))
